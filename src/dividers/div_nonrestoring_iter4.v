// =====================================================================
//  div_nonrestoring_iter4.v
//  4-bit iterative non-restoring divider (4 cycles).
...
module div_nonrestoring_iter4(input clk, input rst, input start, input [3:0] a, input [3:0] b,
    output reg [3:0] quotient, output reg [3:0] remainder,
    ...
    reg [3:0] q, dividend, divisor; reg [4:0] rem; reg [2:0] i;
    wire dz_now = ~(|b);
    assign divide_by_zero = dz_now;
    wire [4:0] sh = {rem[3:0], dividend[3]};
    wire [4:0] tr; wire bo;
    divstep_sub5 st(.a(sh), .b({1'b0,divisor}), .diff(tr), .bout(bo));
    wire qbit = ~bo;
    always @(posedge clk) begin
        if (rst) begin busy<=0; done<=0; valid<=0; q<=0; rem<=0; i<=0; quotient<=0; remainder<=0; end
        else if (start) begin
            if (dz_now) begin quotient<={4{1'b1}}; remainder<=a; valid<=0; done<=1; busy<=0; end
            ...
            i <= i + 1'b1;
            if (i == 3) begin busy<=0; done<=1; valid<=1;
                quotient  <= {q[2:0], qbit};
                remainder <= (qbit ? tr[3:0] : sh[3:0]); end
        end else done<=0;
    end
endmodule

module divstep_sub5(input [4:0] a, input [4:0] b, output [4:0] diff, output bout);
