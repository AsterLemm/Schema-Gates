// =====================================================================
//  remainder_restoring8.v
//  8-bit restoring-division remainder unit.
...
module remainder_restoring8(input [7:0] a, input [7:0] b, output [7:0] remainder, output divide_by_zero, output valid, output busy, output done);
    // define a input 80.160.255   // define b input 80.200.255   // define remainder output 120.255.160
    wire dz=~(|b);
    wire [7:0] q_unused, r;
    wire d2,o2,v2,b2,dn2;
    div_ucore8 dv(.a(a),.b(b),.quotient(q_unused),.remainder(r),.divide_by_zero(d2),.overflow(o2),.valid(v2),.busy(b2),.done(dn2));
    assign remainder = dz ? a : r;
    assign divide_by_zero=dz; assign valid=~dz; assign busy=1'b0; assign done=1'b1;
endmodule

module div_ucore8(input [7:0] a, input [7:0] b,
