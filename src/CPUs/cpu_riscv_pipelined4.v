// =====================================================================
//  cpu_riscv_pipelined4.v
//  4-bit RV-LITE 5-STAGE PIPELINED CPU (IF / ID / EX / MEM / WB).
...
// --- cpu_riscv_pipelined4_decode : fields, instruction classes, ALU path selects ---
module cpu_riscv_pipelined4_decode(
    input  wire [31:0] ir,
    output wire [3:0]  op,
    output wire [3:0]  rd,
    output wire [3:0]  rs1,
    ...
    assign immv = imm16[3:0];   // low bits of the signed imm16
...
module cpu_riscv_pipelined4_regfile(
    ...
    input  wire [3:0] wb_val,
    output wire [3:0] rdata1,
    output wire [3:0] rdata2,
    input  wire [3:0]  dbg_sel,
    output wire [3:0] dbg_data
);
    reg [3:0] x [0:15];        // x0 reads as zero

    assign rdata1 = (raddr1 == 4'd0) ? {4{1'b0}}
                  : (wb_v && wb_rw && wb_rd == raddr1) ? wb_val
                  : x[raddr1];
    assign rdata2 = (raddr2 == 4'd0) ? {4{1'b0}}
                  : (wb_v && wb_rw && wb_rd == raddr2) ? wb_val
                  : x[raddr2];
    assign dbg_data = (dbg_sel == 4'd0) ? {4{1'b0}} : x[dbg_sel];
...
// --- cpu_riscv_pipelined4_fwd : EX forwarding (EX/MEM and MEM/WB sources) ---
module cpu_riscv_pipelined4_fwd(
    input  wire [3:0] idex_rs1v,
    input  wire [3:0] idex_rs2v,
    input  wire [3:0]  idex_rs1,
    input  wire [3:0]  idex_rs2,
    input  wire        exmem_v,
    input  wire        exmem_rw,
    input  wire [3:0]  exmem_rd,
    input  wire [3:0] exmem_val,
    input  wire        memwb_v,
    input  wire        memwb_rw,
    input  wire [3:0]  memwb_rd,
    input  wire [3:0] memwb_val,
    output wire [3:0] fwd1,
    output wire [3:0] fwd2
);
...
// --- cpu_riscv_pipelined4_alu_addsub : ADD / SUB path (leaf of cpu_riscv_pipelined4_alu) ---
module cpu_riscv_pipelined4_alu_addsub(
    input  wire [3:0] a,
    input  wire [3:0] b,
    input  wire        sub_en,
    output wire [3:0] y
...
// --- cpu_riscv_pipelined4_alu_logic : AND / OR / XOR path (leaf of cpu_riscv_pipelined4_alu) ---
module cpu_riscv_pipelined4_alu_logic(
    input  wire [3:0] a,
    input  wire [3:0] b,
    input  wire [1:0]  lsel,
    output wire [3:0] y
...
// --- cpu_riscv_pipelined4_alu_shift : SLL / SRL / SRA path (leaf of cpu_riscv_pipelined4_alu) ---
module cpu_riscv_pipelined4_alu_shift(
    input  wire [3:0] a,
    input  wire [1:0] n,
    ...
    wire signed [3:0] sra_a = $signed(a);
    wire        [3:0] sra_y = sra_a >>> n;
    assign y = (funct == 4'd5) ? (a <<  n)
             : (funct == 4'd6) ? (a >>  n)
             :                  sra_y;
endmodule

// --- cpu_riscv_pipelined4_alu_cmp : SLT / SLTU path (leaf of cpu_riscv_pipelined4_alu) ---
module cpu_riscv_pipelined4_alu_cmp(
    input  wire [3:0] a,
    input  wire [3:0] b,
    input  wire [3:0]  funct,
    output wire [3:0] y
...
module cpu_riscv_pipelined4_alu(
    ...
    input  wire        gate_cmp,
    output wire [3:0] alu_y
);
    wire [3:0] alu_b = is_alur ? rs2v : immv;

    wire [3:0] add_a = rs1v & {4{gate_add}};
    wire [3:0] add_b = alu_b & {4{gate_add}};
    wire           add_sub_en = (funct == 4'd1);   // R-type SUB
    wire [3:0] add_y;
    cpu_riscv_pipelined4_alu_addsub u_path_add(.a(add_a), .b(add_b), .sub_en(add_sub_en), .y(add_y));

    wire [3:0] log_a = rs1v & {4{gate_logic}};
    wire [3:0] log_b = alu_b & {4{gate_logic}};
    wire [3:0] log_y;
    cpu_riscv_pipelined4_alu_logic u_path_log(.a(log_a), .b(log_b), .lsel(lsel), .y(log_y));

    wire [3:0] shf_a = rs1v & {4{gate_shift}};
    wire [1:0] shf_n = rs2v[1:0] & {2{gate_shift}};
    wire [3:0] shf_y;
    cpu_riscv_pipelined4_alu_shift u_path_shf(.a(shf_a), .n(shf_n), .funct(funct), .y(shf_y));

    wire [3:0] cmp_a = rs1v & {4{gate_cmp}};
    wire [3:0] cmp_b = alu_b & {4{gate_cmp}};
    wire [3:0] cmp_y;
    cpu_riscv_pipelined4_alu_cmp u_path_cmp(.a(cmp_a), .b(cmp_b), .funct(funct), .y(cmp_y));

    // path merge: one-hot by construction (decoder-style front mux)
    assign alu_y = (add_y & {4{gate_add}})
                 | (log_y & {4{gate_logic}})
                 | (shf_y & {4{gate_shift}})
                 | (cmp_y & {4{gate_cmp}});
endmodule

// --- cpu_riscv_pipelined4_bcmp : gated branch comparator ---
module cpu_riscv_pipelined4_bcmp(
    input  wire [3:0] fwd1,
    input  wire [3:0] fwd2,
    ...
    wire [3:0] bcm_a = fwd1 & {4{gate_bcmp}};
    wire [3:0] bcm_b = fwd2 & {4{gate_bcmp}};
...
// --- cpu_riscv_pipelined4_exctl : branch/jump redirect + WB value select (EX stage) ---
module cpu_riscv_pipelined4_exctl(
    input  wire        idex_v,
    ...
    wire [3:0] ex_pcret = ex_pc1[3:0];
    assign ex_wbv = idex_lui              ? idex_imm16[3:0]
                          : (idex_jal | idex_jalr) ? ex_pcret
                          : idex_outi              ? fwd1
                          :                          alu_y;
endmodule

// --- cpu_riscv_pipelined4_hazard : load-use interlock detect ---
module cpu_riscv_pipelined4_hazard(
    input  wire        idex_v,
...
// --- cpu_riscv_pipelined4_dmem : 16-word data RAM (sync write, async read) ---
module cpu_riscv_pipelined4_dmem(
    input  wire        clk,
    input  wire        we,
    input  wire [3:0]  addr,
    input  wire [3:0] wdata,
...
module cpu_riscv_pipelined4(
    ...
    wire [1:0] lsel;
    wire reg_write;
    cpu_riscv_pipelined4_decode u_decode(
        .ir(ifid_ir),
    ...
    wire [3:0] rf1, rf2;
    cpu_riscv_pipelined4_regfile u_regfile(
        .clk(clk),
    ...
    reg         idex_v;
    reg  [7:0]  idex_pc;
    reg [3:0] idex_rs1v, idex_rs2v, idex_imm;
    ...
    reg  [3:0]  exmem_rd;
    reg [3:0] exmem_val, exmem_sval;   // ALU/wb value, store data

    wire [3:0] fwd1, fwd2;
    cpu_riscv_pipelined4_fwd u_fwd(
    ...
    wire [3:0] alu_y;
    cpu_riscv_pipelined4_alu u_alu(
    ...
    wire btaken;
    cpu_riscv_pipelined4_bcmp u_bcmp(
    ...
    wire [3:0] ex_wbv;
    cpu_riscv_pipelined4_exctl u_exctl(
    ...
    cpu_riscv_pipelined4_hazard u_hazard(
    ...
    // =================================================================
    wire [3:0] mem_rd;
    cpu_riscv_pipelined4_dmem u_dmem(
        .clk(clk),
