// =====================================================================
//  cpu_arm8.v
//  8-bit ARM-FLAVOURED CPU: full conditional execution (NZCV),
...
// --- cpu_arm8_cond : ARM condition check (gates EVERY instruction) ---
module cpu_arm8_cond(
    input  wire [3:0] cond,
    input  wire       fN,
    input  wire       fZ,
    input  wire       fC,
...
// --- cpu_arm8_decode : field extraction + condition-gated class decode ---
module cpu_arm8_decode(
    input  wire [31:0] ir,
...
// --- cpu_arm8_shifter : inline barrel shifter on operand 2 ---
// (operand-isolated: only live for register-form data processing)
module cpu_arm8_shifter(
    input  wire [7:0] rmv,
    input  wire [4:0]  shamt,
    input  wire [1:0]  shtyp,
    input  wire        sh_en,
    input  wire        fC,
    output reg  [7:0] sh_out,
    output reg         sh_cout
);
    wire [7:0] sh_in = rmv & {8{sh_en}};
        ...
        case (shtyp)
            2'd0: begin                                     // LSL
                ...
                sh_cout = (sh_n == 5'd0) ? fC
                        : (sh_n <= 5'd8) ? sh_in[8-sh_n] : 1'b0;
            end
            2'd1: begin                                     // LSR
                sh_out  = sh_in >> sh_n;
                sh_cout = (sh_n == 5'd0) ? fC
                        : (sh_n <= 5'd8) ? sh_in[sh_n-5'd1] : 1'b0;
            end
            2'd2: begin                                     // ASR
                sh_out  = $signed(sh_in) >>> sh_n;
                sh_cout = (sh_n == 5'd0) ? fC
                        : (sh_n <= 5'd8) ? sh_in[sh_n-5'd1] : sh_in[7];
            end
            default: begin                                  // ROR
                sh_out  = (sh_in >> sh_n[2:0]) | (sh_in << (8 - sh_n[2:0]));
                sh_cout = (sh_n == 5'd0) ? fC : sh_out[7];
            end
        endcase
    end
endmodule

// --- cpu_arm8_alu_arith : unified A+B+cin adder, ARM C/V convention ---
// (leaf of cpu_arm8_alu)
module cpu_arm8_alu_arith(
    input  wire [7:0] aa,
    input  wire [7:0] ab,
    input  wire [3:0]  op4,
    input  wire        fC,
    ...
    wire cin   = (op4 == 4'd2) | (op4 == 4'd3) | (op4 == 4'd12) ? 1'b1
               ...
               : 1'b0;
    wire [7:0] adA = inv_a ? ~aa : aa;
    wire [7:0] adB = inv_b ? ~ab : ab;
    wire [8:0] adS = {1'b0, adA} + {1'b0, adB} + {{8{1'b0}}, cin};
    assign ar_y = adS[7:0];
    assign ar_c = adS[8];                 // ARM: C = NOT borrow on subtract
    assign ar_v = (adA[7] == adB[7]) & (ar_y[7] != adA[7]);
endmodule

// --- cpu_arm8_alu_logic : AND/EOR/ORR/MOV/MVN/BIC + TST/TEQ (leaf of cpu_arm8_alu) ---
module cpu_arm8_alu_logic(
    input  wire [7:0] la,
    input  wire [7:0] lb,
    input  wire [3:0]  op4,
    output reg  [7:0] lg_y
...
// --- cpu_arm8_alu : data-processing ALU (operand-isolated arith + logic) ---
module cpu_arm8_alu(
    input  wire [7:0] rnv,
    input  wire [7:0] op2,
    input  wire [3:0]  op4,
    ...
    output wire [7:0] dp_y,
    output wire        ar_c,
    output wire        ar_v
);
    wire [7:0] aa = rnv & {8{sel_arith}};
    wire [7:0] ab = op2 & {8{sel_arith}};
    wire [7:0] ar_y;
    cpu_arm8_alu_arith u_arith(.aa(aa), .ab(ab), .op4(op4), .fC(fC),
                          .ar_y(ar_y), .ar_c(ar_c), .ar_v(ar_v));

    wire [7:0] la = rnv & {8{sel_logic}};
    wire [7:0] lb = op2 & {8{sel_logic}};
    wire [7:0] lg_y;
    cpu_arm8_alu_logic u_logic(.la(la), .lb(lb), .op4(op4), .lg_y(lg_y));

    assign dp_y = sel_arith ? ar_y : lg_y;
endmodule

// --- cpu_arm8_regfile : 16x8 register file, r14 = LR by convention ---
...
module cpu_arm8_regfile(
    ...
    reg [7:0] r [0:15];        // r14 = LR by convention
...
// --- cpu_arm8_dmem : 32-word data RAM (sync write, async read) ---
module cpu_arm8_dmem(
    input  wire        clk,
    input  wire        we,
    input  wire [4:0]  addr,
    input  wire [7:0] wdata,
...
// --- cpu_arm8_flags : NZCV status register (ARM S-bit semantics) ---
module cpu_arm8_flags(
    input  wire        clk,
    input  wire        rst,
    input  wire        we,
    input  wire [7:0] dp_y,
...
module cpu_arm8(
    ...
    cpu_arm8_cond u_cond(
        .cond(cond), .fN(fN), .fZ(fZ), .fC(fC), .fV(fV),
        .cond_pass(cond_pass)
    );
    cpu_arm8_decode u_decode(
    ...
    wire [7:0] rnv, rmv, rdv, dmem_rd, dp_y;
    wire [7:0] linkv = pc1[7:0];   // BL: r14 <= pc+1
    wire rf_we = wr_gate & (dp_wr | is_ldr | is_bl);
    wire [3:0] rf_waddr = is_bl ? 4'd14 : rd;
    wire [7:0] rf_wdata = is_bl  ? linkv
                          : is_ldr ? dmem_rd
                          :          dp_y;
    cpu_arm8_regfile u_regfile(
        .clk(clk),
    ...
    wire [7:0] imm8w = imm8[7:0];
    wire [7:0] op2 = (cls == 2'b00) ? sh_out : imm8w;
