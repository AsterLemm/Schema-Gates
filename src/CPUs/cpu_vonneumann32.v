// =====================================================================
//  cpu_vonneumann32.v
//  32-bit VON NEUMANN accumulator CPU (multicycle, unified memory).
//  One 16x32 memory holds code AND data behind a single port,
...
// --- cpu_vonneumann32_mem : the unified von Neumann memory (16x32) ---
...
// --- cpu_vonneumann32_alu_arith : ADD / SUB with carry view (leaf of cpu_vonneumann32_alu) ---
module cpu_vonneumann32_alu_arith(
    input  wire [31:0] ar_a,
    input  wire [31:0] ar_b,
    input  wire        is_sub,
    output wire [32:0]   ar_sum
);
    assign ar_sum = is_sub ? ({1'b0, ar_a} - {1'b0, ar_b})
                           : ({1'b0, ar_a} + {1'b0, ar_b});
endmodule

// --- cpu_vonneumann32_alu_logic : AND / OR / XOR (leaf of cpu_vonneumann32_alu) ---
module cpu_vonneumann32_alu_logic(
...
// --- cpu_vonneumann32_alu_shift : SHL / SHR with carry-out (leaf of cpu_vonneumann32_alu) ---
module cpu_vonneumann32_alu_shift(
    input  wire [31:0] sh_a,
    input  wire [3:0]  opcode,
    output wire [31:0] sh_y,
    output wire        sh_c
);
    assign sh_y = (opcode == 4'hC) ? {sh_a[30:0], 1'b0}
                :                    {1'b0, sh_a[31:1]};
    assign sh_c = (opcode == 4'hC) ? sh_a[31] : sh_a[0];
endmodule

// --- cpu_vonneumann32_alu : operand-isolated accumulator ALU (flagship technique) ---
// Each path's inputs are ANDed with its select so unused paths hold 0.
module cpu_vonneumann32_alu(
    input  wire [31:0] acc,
    ...
    wire [31:0] ar_a = acc       & {32{sel_arith}};
    wire [31:0] ar_b = mem_rdata & {32{sel_arith}};
    cpu_vonneumann32_alu_arith u_arith(.ar_a(ar_a), .ar_b(ar_b), .is_sub(is_sub),
                          .ar_sum(ar_sum));

    wire [31:0] lg_a = acc       & {32{sel_logic}};
    wire [31:0] lg_b = mem_rdata & {32{sel_logic}};
    cpu_vonneumann32_alu_logic u_logic(.lg_a(lg_a), .lg_b(lg_b), .opcode(opcode),
                          .lg_y(lg_y));

    wire [31:0] sh_a = acc & {32{sel_shift}};
    cpu_vonneumann32_alu_shift u_shift(.sh_a(sh_a), .opcode(opcode),
                          .sh_y(sh_y), .sh_c(sh_c));
endmodule

module cpu_vonneumann32(
    input  wire        clk,
    input  wire        rst,
    input  wire        run,
    ...
    wire [31:0] mem_wdata = (~run) ? prog_data : acc;
    wire [31:0] mem_rdata;
    cpu_vonneumann32_mem u_mem(
        .clk(clk),
    ...
    always @(posedge clk) begin
        out_valid <= 1'b0;
        if (rst) begin
            state <= ST_FETCH; pc <= 4'd0; acc <= {32{1'b0}};
            ...
            case (state)
                ...
                ST_EXEC: begin
                    ...
                    case (cur_opcode)
                        ...
                        4'h3: begin acc <= ar_sum[31:0]; carry <= ar_sum[32]; end // ADD
                        4'h4: begin acc <= ar_sum[31:0]; carry <= ar_sum[32]; end // SUB (carry = borrow)
                        4'h5, 4'h6, 4'h7: acc <= lg_y;            // AND OR XOR
                        4'h8: acc <= {{28{1'b0}}, cur_operand}; // LDI
