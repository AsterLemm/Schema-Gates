// =====================================================================
//  cpu_arm4.v
//  4-bit ARM-FLAVOURED CPU: full conditional execution (NZCV),
...
// --- cpu_arm4_cond : ARM condition check (gates EVERY instruction) ---
module cpu_arm4_cond(
    input  wire [3:0] cond,
    input  wire       fN,
    input  wire       fZ,
    input  wire       fC,
...
// --- cpu_arm4_decode : field extraction + condition-gated class decode ---
module cpu_arm4_decode(
    input  wire [31:0] ir,
...
// --- cpu_arm4_shifter : inline barrel shifter on operand 2 ---
// (operand-isolated: only live for register-form data processing)
module cpu_arm4_shifter(
    input  wire [3:0] rmv,
    input  wire [4:0]  shamt,
    input  wire [1:0]  shtyp,
    input  wire        sh_en,
    input  wire        fC,
    output reg  [3:0] sh_out,
    output reg         sh_cout
);
    wire [3:0] sh_in = rmv & {4{sh_en}};
        ...
        case (shtyp)
            2'd0: begin                                     // LSL
                ...
                sh_cout = (sh_n == 5'd0) ? fC
                        : (sh_n <= 5'd4) ? sh_in[4-sh_n] : 1'b0;
            end
            2'd1: begin                                     // LSR
                sh_out  = sh_in >> sh_n;
                sh_cout = (sh_n == 5'd0) ? fC
                        : (sh_n <= 5'd4) ? sh_in[sh_n-5'd1] : 1'b0;
            end
            2'd2: begin                                     // ASR
                sh_out  = $signed(sh_in) >>> sh_n;
                sh_cout = (sh_n == 5'd0) ? fC
                        : (sh_n <= 5'd4) ? sh_in[sh_n-5'd1] : sh_in[3];
            end
            default: begin                                  // ROR
                sh_out  = (sh_in >> sh_n[1:0]) | (sh_in << (4 - sh_n[1:0]));
                sh_cout = (sh_n == 5'd0) ? fC : sh_out[3];
            end
        endcase
    end
endmodule

// --- cpu_arm4_alu_arith : unified A+B+cin adder, ARM C/V convention ---
// (leaf of cpu_arm4_alu)
module cpu_arm4_alu_arith(
    input  wire [3:0] aa,
    input  wire [3:0] ab,
    input  wire [3:0]  op4,
    input  wire        fC,
    ...
    wire cin   = (op4 == 4'd2) | (op4 == 4'd3) | (op4 == 4'd12) ? 1'b1
               ...
               : 1'b0;
    wire [3:0] adA = inv_a ? ~aa : aa;
    wire [3:0] adB = inv_b ? ~ab : ab;
    wire [4:0] adS = {1'b0, adA} + {1'b0, adB} + {{4{1'b0}}, cin};
    assign ar_y = adS[3:0];
    assign ar_c = adS[4];                 // ARM: C = NOT borrow on subtract
    assign ar_v = (adA[3] == adB[3]) & (ar_y[3] != adA[3]);
endmodule

// --- cpu_arm4_alu_logic : AND/EOR/ORR/MOV/MVN/BIC + TST/TEQ (leaf of cpu_arm4_alu) ---
module cpu_arm4_alu_logic(
    input  wire [3:0] la,
    input  wire [3:0] lb,
    input  wire [3:0]  op4,
    output reg  [3:0] lg_y
...
// --- cpu_arm4_alu : data-processing ALU (operand-isolated arith + logic) ---
module cpu_arm4_alu(
    input  wire [3:0] rnv,
    input  wire [3:0] op2,
    input  wire [3:0]  op4,
    ...
    output wire [3:0] dp_y,
    output wire        ar_c,
    output wire        ar_v
);
    wire [3:0] aa = rnv & {4{sel_arith}};
    wire [3:0] ab = op2 & {4{sel_arith}};
    wire [3:0] ar_y;
    cpu_arm4_alu_arith u_arith(.aa(aa), .ab(ab), .op4(op4), .fC(fC),
                          .ar_y(ar_y), .ar_c(ar_c), .ar_v(ar_v));

    wire [3:0] la = rnv & {4{sel_logic}};
    wire [3:0] lb = op2 & {4{sel_logic}};
    wire [3:0] lg_y;
    cpu_arm4_alu_logic u_logic(.la(la), .lb(lb), .op4(op4), .lg_y(lg_y));

    assign dp_y = sel_arith ? ar_y : lg_y;
endmodule

// --- cpu_arm4_regfile : 16x4 register file, r14 = LR by convention ---
...
module cpu_arm4_regfile(
    ...
    reg [3:0] r [0:15];        // r14 = LR by convention
...
// --- cpu_arm4_dmem : 32-word data RAM (sync write, async read) ---
module cpu_arm4_dmem(
    input  wire        clk,
    input  wire        we,
    input  wire [4:0]  addr,
    input  wire [3:0] wdata,
...
// --- cpu_arm4_flags : NZCV status register (ARM S-bit semantics) ---
module cpu_arm4_flags(
    input  wire        clk,
    input  wire        rst,
    input  wire        we,
    input  wire [3:0] dp_y,
...
module cpu_arm4(
    ...
    cpu_arm4_cond u_cond(
        .cond(cond), .fN(fN), .fZ(fZ), .fC(fC), .fV(fV),
        .cond_pass(cond_pass)
    );
    cpu_arm4_decode u_decode(
    ...
    wire [3:0] rnv, rmv, rdv, dmem_rd, dp_y;
    wire [3:0] linkv = pc1[3:0];   // BL: r14 <= pc+1
    wire rf_we = wr_gate & (dp_wr | is_ldr | is_bl);
    wire [3:0] rf_waddr = is_bl ? 4'd14 : rd;
    wire [3:0] rf_wdata = is_bl  ? linkv
                          : is_ldr ? dmem_rd
                          :          dp_y;
    cpu_arm4_regfile u_regfile(
        .clk(clk),
    ...
    wire [3:0] imm8w = imm8[3:0];
    wire [3:0] op2 = (cls == 2'b00) ? sh_out : imm8w;
