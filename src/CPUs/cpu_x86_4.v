// =====================================================================
//  cpu_x86_4.v
//  4-bit x86-FLAVOURED CISC CPU (two-operand destructive ISA).
...
// --- cpu_x86_4_decode : fields + instruction class decode ---
module cpu_x86_4_decode(
    input  wire [15:0] ir,
    output wire [3:0]  op,
...
// --- cpu_x86_4_alu_arith : add/sub path with carry-chain CF/OF view ---
// (leaf of cpu_x86_4_alu)
module cpu_x86_4_alu_arith(
    input  wire [3:0] ar_a,
    input  wire [3:0] ar_b,
    input  wire        ar_subop,
    output wire [3:0] ar_y,
    output wire        ar_c,
    output wire        ar_v
);
    wire [4:0] ar_full = ar_subop ? ({1'b0, ar_a} - {1'b0, ar_b})
                                    : ({1'b0, ar_a} + {1'b0, ar_b});
    assign ar_y = ar_full[3:0];
    assign ar_c = ar_full[4];                       // carry / borrow
    assign ar_v = ar_subop ? ((ar_a[3] != ar_b[3]) & (ar_y[3] != ar_a[3]))
                         : ((ar_a[3] == ar_b[3]) & (ar_y[3] != ar_a[3]));
endmodule

// --- cpu_x86_4_alu_logic : AND / OR / XOR / NOT path (leaf of cpu_x86_4_alu) ---
module cpu_x86_4_alu_logic(
    input  wire [3:0] lg_a,
    input  wire [3:0] lg_b,
    input  wire        is_and,
    input  wire        is_or,
    input  wire        is_xor,
    output wire [3:0] lg_y
);
...
// --- cpu_x86_4_alu_shift : single-position shifts, x86 flag semantics ---
// (leaf of cpu_x86_4_alu)
module cpu_x86_4_alu_shift(
    input  wire [3:0] sh_a,
    input  wire [1:0]  msel,
    output wire [3:0] sh_y,
    output wire        sh_c,
    output wire        sh_v
);
    assign sh_y = (msel == 2'd0) ? {sh_a[2:0], 1'b0}        // SHL
                : (msel == 2'd1) ? {1'b0, sh_a[3:1]}        // SHR
                : (msel == 2'd2) ? {sh_a[3], sh_a[3:1]}    // SAR
                :                  {sh_a[2:0], sh_a[3]};   // ROL
    assign sh_c = (msel == 2'd0) ? sh_a[3]
              : (msel == 2'd3) ? sh_a[3]
              :                  sh_a[0];
    assign sh_v = (msel == 2'd0) ? (sh_y[3] ^ sh_c)    // SHL: OF = CF^MSB(result)
              : (msel == 2'd1) ? sh_a[3]             // SHR: OF = old MSB
              : (msel == 2'd3) ? (sh_y[3] ^ sh_c)    // ROL
              :                  1'b0;                // SAR: OF = 0
endmodule

// --- cpu_x86_4_alu : operand-isolated 3-path ALU (flagship technique) ---
// operand muxes for the unary ops (INC/DEC use +/-1, NEG uses 0-r)
// live here; each path's inputs are ANDed with its select line.
module cpu_x86_4_alu(
    input  wire [3:0] rv,
    input  wire [3:0] mv,
    input  wire [1:0]  msel,
    input  wire        is_sub,
    ...
    wire [3:0] ar_a = (is_unary ? ((msel == 2'd3) ? {4{1'b0}} : rv) : rv)
                        & {4{sel_arith}};
    wire [3:0] ar_b = (is_unary ? ((msel == 2'd3) ? rv : {{3{1'b0}}, 1'b1}) : mv)
                        & {4{sel_arith}};
    wire ar_subop = is_sub | is_cmp
                  | (is_unary & ((msel == 2'd1) | (msel == 2'd3))); // DEC NEG
    cpu_x86_4_alu_arith u_arith(.ar_a(ar_a), .ar_b(ar_b), .ar_subop(ar_subop),
                          .ar_y(ar_y), .ar_c(ar_c), .ar_v(ar_v));

    // logic path
    wire [3:0] lg_a = rv & {4{sel_logic}};
    wire [3:0] lg_b = mv & {4{sel_logic}};
    cpu_x86_4_alu_logic u_logic(.lg_a(lg_a), .lg_b(lg_b),
                          .is_and(is_and), .is_or(is_or), .is_xor(is_xor),
                          .lg_y(lg_y));

    // shift path (single position, x86 flag semantics)
    wire [3:0] sh_a = rv & {4{sel_shift}};
    cpu_x86_4_alu_shift u_shift(.sh_a(sh_a), .msel(msel),
                          .sh_y(sh_y), .sh_c(sh_c), .sh_v(sh_v));
endmodule

// --- cpu_x86_4_cond : x86 Jcc condition table ---
module cpu_x86_4_cond(
    input  wire [3:0]  cond,
    input  wire        zf,
    input  wire        cf,
    input  wire        sf,
    ...
    always @(*) begin case (cond)
        ...
        4'd13: ctaken = (cxv != {4{1'b0}});  // LOOP looks at CX-1... see EXEC
        default: ctaken = 1'b0;          // 14/15 never
    endcase end
endmodule

// --- cpu_x86_4_regfile : AX/BX/CX/DX (single muxed write port) ---
// all writing instruction classes are mutually exclusive, so one
// port (driven by cpu_x86_4_wbsel) carries every result; LOOP writes CX.
module cpu_x86_4_regfile(
    input  wire        clk,
    input  wire        rst,
    input  wire        we,
    ...
    reg [3:0] regs [0:3];      // 0 AX, 1 BX, 2 CX, 3 DX
...
module cpu_x86_4_ram(
    ...
    output wire [3:0] rdata_s,
    output wire [3:0] rdata_i
);
    reg [3:0] ram [0:31];     // unified data + descending stack
...
// --- cpu_x86_4_wbsel : register write-back select (one port, many sources) ---
module cpu_x86_4_wbsel(
    input  wire        is_movrr,
    input  wire        is_movi,
    input  wire        is_add,
    input  wire        is_sub,
    ...
    input  wire [3:0] lg_y,
    input  wire [3:0] sh_y,
    input  wire [3:0] ram_rd,
    input  wire [3:0] stk_top,
    input  wire [3:0] cx_dec,
    output wire        wb_en,
    output wire [1:0]  wb_addr,
    output wire [3:0] wb_data
);
            ...
            case (op)
                ...
                4'h8: case (msel)                              // unary
                    2'd0, 2'd1: begin                          // INC DEC
                        zf <= (ar_y == {4{1'b0}}); sf <= ar_y[3]; vf <= ar_v;
                    end                                        // CF PRESERVED (x86 quirk)
                    2'd2: ;                                    // NOT: no flags (x86 quirk)
                    2'd3: begin                                // NEG = 0 - r
                        cf <= (rv != {4{1'b0}});            // x86: CF=0 only if src was 0
                        zf <= (ar_y == {4{1'b0}}); sf <= ar_y[3]; vf <= ar_v;
                    end
                endcase
...
// --- cpu_x86_4_pcnext : next-PC select (default +1; flow ops override) ---
module cpu_x86_4_pcnext(
    input  wire [7:0]  pc,
    input  wire [7:0]  imm8,
    input  wire [3:0] stk_top,
    input  wire        is_call,
...
module cpu_x86_4(
    ...
    wire [3:0] rv, mv, cxv, rf_dbg;
    wire [3:0] ar_y, lg_y, sh_y, ram_rd, stk_top;
    wire ar_c, ar_v, sh_c, sh_v;
    wire [3:0] cx_dec = cxv - {{3{1'b0}}, 1'b1};
    wire loop_take = (cx_dec != {4{1'b0}});
    wire wb_en;
    wire [1:0] wb_addr;
    wire [3:0] wb_data;
    cpu_x86_4_wbsel u_wbsel(
        .is_movrr(is_movrr), .is_movi(is_movi), .is_add(is_add),
        .is_sub(is_sub), .is_and(is_and), .is_or(is_or),
    ...
    wire [3:0] ram_wdata = is_call ? pc1w[3:0] : rv;
    cpu_x86_4_ram u_ram(
        .clk(clk),
