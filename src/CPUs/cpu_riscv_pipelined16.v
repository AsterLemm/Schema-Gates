...
// --- cpu_riscv_pipelined16_decode : fields, instruction classes, ALU path selects ---
module cpu_riscv_pipelined16_decode(
    input  wire [31:0] ir,
    output wire [3:0]  op,
    output wire [3:0]  rd,
    output wire [3:0]  rs1,
...
module cpu_riscv_pipelined16_regfile(
    ...
    assign rdata1 = (raddr1 == 4'd0) ? {16{1'b0}}
                  : (wb_v && wb_rw && wb_rd == raddr1) ? wb_val
                  : x[raddr1];
    assign rdata2 = (raddr2 == 4'd0) ? {16{1'b0}}
...
// --- cpu_riscv_pipelined16_fwd : EX forwarding (EX/MEM and MEM/WB sources) ---
module cpu_riscv_pipelined16_fwd(
    input  wire [15:0] idex_rs1v,
    input  wire [15:0] idex_rs2v,
    input  wire [3:0]  idex_rs1,
    input  wire [3:0]  idex_rs2,
    input  wire        exmem_v,
    input  wire        exmem_rw,
    input  wire [3:0]  exmem_rd,
    input  wire [15:0] exmem_val,
    input  wire        memwb_v,
    input  wire        memwb_rw,
    input  wire [3:0]  memwb_rd,
    input  wire [15:0] memwb_val,
    output wire [15:0] fwd1,
    output wire [15:0] fwd2
);
...
// --- cpu_riscv_pipelined16_alu_addsub : ADD / SUB path (leaf of cpu_riscv_pipelined16_alu) ---
module cpu_riscv_pipelined16_alu_addsub(
    input  wire [15:0] a,
    input  wire [15:0] b,
    input  wire        sub_en,
    output wire [15:0] y
...
// --- cpu_riscv_pipelined16_alu_logic : AND / OR / XOR path (leaf of cpu_riscv_pipelined16_alu) ---
module cpu_riscv_pipelined16_alu_logic(
    input  wire [15:0] a,
    input  wire [15:0] b,
    input  wire [1:0]  lsel,
    output wire [15:0] y
...
// --- cpu_riscv_pipelined16_alu_shift : SLL / SRL / SRA path (leaf of cpu_riscv_pipelined16_alu) ---
module cpu_riscv_pipelined16_alu_shift(
    input  wire [15:0] a,
    input  wire [3:0] n,
    ...
    wire signed [15:0] sra_a = $signed(a);
    wire        [15:0] sra_y = sra_a >>> n;
    assign y = (funct == 4'd5) ? (a <<  n)
             : (funct == 4'd6) ? (a >>  n)
             :                  sra_y;
endmodule

// --- cpu_riscv_pipelined16_alu_cmp : SLT / SLTU path (leaf of cpu_riscv_pipelined16_alu) ---
module cpu_riscv_pipelined16_alu_cmp(
    input  wire [15:0] a,
    input  wire [15:0] b,
    input  wire [3:0]  funct,
    output wire [15:0] y
...
// --- cpu_riscv_pipelined16_alu : operand-isolated 4-path ALU (flagship technique) ---
// gate_X low -> that path's operands are forced to zero, so its
// internal logic does not toggle while another path computes.
module cpu_riscv_pipelined16_alu(
    input  wire [15:0] rs1v,
    input  wire [15:0] rs2v,
    input  wire [15:0] immv,
    input  wire        is_alur,
    input  wire [3:0]  funct,
    input  wire [1:0]  lsel,
    input  wire        gate_add,
    ...
    wire [15:0] add_y;
    cpu_riscv_pipelined16_alu_addsub u_path_add(.a(add_a), .b(add_b), .sub_en(add_sub_en), .y(add_y));

    wire [15:0] log_a = rs1v & {16{gate_logic}};
    wire [15:0] log_b = alu_b & {16{gate_logic}};
    wire [15:0] log_y;
    cpu_riscv_pipelined16_alu_logic u_path_log(.a(log_a), .b(log_b), .lsel(lsel), .y(log_y));

    wire [15:0] shf_a = rs1v & {16{gate_shift}};
    wire [3:0] shf_n = rs2v[3:0] & {4{gate_shift}};
    wire [15:0] shf_y;
    cpu_riscv_pipelined16_alu_shift u_path_shf(.a(shf_a), .n(shf_n), .funct(funct), .y(shf_y));

    wire [15:0] cmp_a = rs1v & {16{gate_cmp}};
    wire [15:0] cmp_b = alu_b & {16{gate_cmp}};
    wire [15:0] cmp_y;
    cpu_riscv_pipelined16_alu_cmp u_path_cmp(.a(cmp_a), .b(cmp_b), .funct(funct), .y(cmp_y));

    // path merge: one-hot by construction (decoder-style front mux)
    assign alu_y = (add_y & {16{gate_add}})
                 | (log_y & {16{gate_logic}})
                 | (shf_y & {16{gate_shift}})
                 | (cmp_y & {16{gate_cmp}});
endmodule

// --- cpu_riscv_pipelined16_bcmp : gated branch comparator ---
module cpu_riscv_pipelined16_bcmp(
    input  wire [15:0] fwd1,
    input  wire [15:0] fwd2,
...
// --- cpu_riscv_pipelined16_exctl : branch/jump redirect + WB value select (EX stage) ---
module cpu_riscv_pipelined16_exctl(
    input  wire        idex_v,
    ...
    assign ex_wbv = idex_lui              ? idex_imm16
                          : (idex_jal | idex_jalr) ? ex_pcret
                          : idex_outi              ? fwd1
                          :                          alu_y;
endmodule

// --- cpu_riscv_pipelined16_hazard : load-use interlock detect ---
module cpu_riscv_pipelined16_hazard(
    input  wire        idex_v,
...
// --- cpu_riscv_pipelined16_dmem : 16-word data RAM (sync write, async read) ---
module cpu_riscv_pipelined16_dmem(
    input  wire        clk,
    input  wire        we,
    input  wire [3:0]  addr,
    input  wire [15:0] wdata,
...
module cpu_riscv_pipelined16(
    ...
    wire [1:0] lsel;
    wire reg_write;
    cpu_riscv_pipelined16_decode u_decode(
        .ir(ifid_ir),
    ...
    wire [15:0] rf1, rf2;
    cpu_riscv_pipelined16_regfile u_regfile(
        .clk(clk),
    ...
    wire [15:0] fwd1, fwd2;
    cpu_riscv_pipelined16_fwd u_fwd(
    ...
    wire [15:0] alu_y;
    cpu_riscv_pipelined16_alu u_alu(
    ...
    wire btaken;
    cpu_riscv_pipelined16_bcmp u_bcmp(
    ...
    wire [15:0] ex_wbv;
    cpu_riscv_pipelined16_exctl u_exctl(
    ...
    cpu_riscv_pipelined16_hazard u_hazard(
    ...
    // =================================================================
    wire [15:0] mem_rd;
    cpu_riscv_pipelined16_dmem u_dmem(
        .clk(clk),
