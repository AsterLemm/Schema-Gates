...
// --- cpu_x86_64_decode : fields + instruction class decode ---
module cpu_x86_64_decode(
    input  wire [15:0] ir,
    output wire [3:0]  op,
...
// --- cpu_x86_64_alu_arith : add/sub path with carry-chain CF/OF view ---
// (leaf of cpu_x86_64_alu)
module cpu_x86_64_alu_arith(
    input  wire [63:0] ar_a,
    input  wire [63:0] ar_b,
    input  wire        ar_subop,
    output wire [63:0] ar_y,
    output wire        ar_c,
    output wire        ar_v
);
    wire [64:0] ar_full = ar_subop ? ({1'b0, ar_a} - {1'b0, ar_b})
                                    : ({1'b0, ar_a} + {1'b0, ar_b});
    assign ar_y = ar_full[63:0];
    assign ar_c = ar_full[64];                       // carry / borrow
    assign ar_v = ar_subop ? ((ar_a[63] != ar_b[63]) & (ar_y[63] != ar_a[63]))
                         : ((ar_a[63] == ar_b[63]) & (ar_y[63] != ar_a[63]));
endmodule

// --- cpu_x86_64_alu_logic : AND / OR / XOR / NOT path (leaf of cpu_x86_64_alu) ---
module cpu_x86_64_alu_logic(
...
// --- cpu_x86_64_alu_shift : single-position shifts, x86 flag semantics ---
// (leaf of cpu_x86_64_alu)
module cpu_x86_64_alu_shift(
...
module cpu_x86_64_cond(
    ...
    always @(*) begin case (cond)
        ...
        4'd13: ctaken = (cxv != {64{1'b0}});  // LOOP looks at CX-1... see EXEC
        default: ctaken = 1'b0;          // 14/15 never
    endcase end
endmodule

// --- cpu_x86_64_regfile : AX/BX/CX/DX (single muxed write port) ---
// all writing instruction classes are mutually exclusive, so one
// port (driven by cpu_x86_64_wbsel) carries every result; LOOP writes CX.
module cpu_x86_64_regfile(
    input  wire        clk,
    input  wire        rst,
    input  wire        we,
...
// --- cpu_x86_64_wbsel : register write-back select (one port, many sources) ---
module cpu_x86_64_wbsel(
    input  wire        is_movrr,
    input  wire        is_movi,
    input  wire        is_add,
    input  wire        is_sub,
    ...
    input  wire [63:0] rv,
    input  wire [63:0] mv,
    input  wire [63:0] ar_y,
    input  wire [63:0] lg_y,
    input  wire [63:0] sh_y,
    input  wire [63:0] ram_rd,
    input  wire [63:0] stk_top,
    input  wire [63:0] cx_dec,
    output wire        wb_en,
    ...
    assign wb_data = is_movrr ? mv                                // MOV r,m
                   : is_movi  ? {{56{1'b0}}, imm8}         // MOV r,imm8
...
// --- cpu_x86_64_pcnext : next-PC select (default +1; flow ops override) ---
module cpu_x86_64_pcnext(
    input  wire [7:0]  pc,
    input  wire [7:0]  imm8,
    input  wire [63:0] stk_top,
    input  wire        is_call,
...
module cpu_x86_64(
    ...
    wire [63:0] rv, mv, cxv, rf_dbg;
    wire [63:0] ar_y, lg_y, sh_y, ram_rd, stk_top;
    wire ar_c, ar_v, sh_c, sh_v;
    wire [63:0] cx_dec = cxv - {{63{1'b0}}, 1'b1};
    wire loop_take = (cx_dec != {64{1'b0}});
    wire wb_en;
    wire [1:0] wb_addr;
    wire [63:0] wb_data;
    cpu_x86_64_wbsel u_wbsel(
        .is_movrr(is_movrr), .is_movi(is_movi), .is_add(is_add),
        .is_sub(is_sub), .is_and(is_and), .is_or(is_or),
    ...
    wire [63:0] ram_wdata = is_call ? {{56{1'b0}}, pc1w} : rv;
    cpu_x86_64_ram u_ram(
        .clk(clk),
