// =====================================================================
//  cpu_stack16.v
//  16-bit ZERO-ADDRESS STACK CPU (Forth-style dual-stack machine).
...
// --- cpu_stack16_decode : fields + instruction class decode ---
module cpu_stack16_decode(
    input  wire [15:0] ir,
    output wire [3:0]  op,
    output wire [11:0] imm12,
    output wire [15:0] pushv,
    ...
    assign pushv = {{4{imm12[11]}}, imm12};   // PUSHI value
...
// --- cpu_stack16_alu_arith : ADD / SUB path (leaf of cpu_stack16_alu) ---
module cpu_stack16_alu_arith(
    input  wire [15:0] al_a,
    input  wire [15:0] al_b,
    input  wire [3:0]  op,
    output wire [15:0] y
);
...
// --- cpu_stack16_alu_logic : AND / OR / XOR path (leaf of cpu_stack16_alu) ---
module cpu_stack16_alu_logic(
    input  wire [15:0] al_a,
    input  wire [15:0] al_b,
    input  wire [3:0]  op,
    output wire [15:0] y
...
// --- cpu_stack16_alu : binop ALU on the two top cells (operand-isolated) ---
module cpu_stack16_alu(
    input  wire [15:0] nos,
    input  wire [15:0] tos,
    input  wire        is_binop,
    input  wire [3:0]  op,
    output wire [15:0] al_y
);
    wire [15:0] al_a = nos & {16{is_binop}};
    wire [15:0] al_b = tos & {16{is_binop}};
    wire [15:0] ar_y, lg_y;
    cpu_stack16_alu_arith u_arith(.al_a(al_a), .al_b(al_b), .op(op), .y(ar_y));
    cpu_stack16_alu_logic u_logic(.al_a(al_a), .al_b(al_b), .op(op), .y(lg_y));
...
// --- cpu_stack16_dstack : 16-deep data-stack spill RAM (cells BELOW nos) ---
module cpu_stack16_dstack(
    input  wire        clk,
    input  wire        we,
    ...
    reg [15:0] dstk [0:15];     // cells BELOW nos
...
// --- cpu_stack16_rstack : 8-deep return stack (the dual-stack signature) ---
module cpu_stack16_rstack(
    input  wire        clk,
    input  wire        we,
...
// --- cpu_stack16_dmem : 32-word data RAM (sync write, async read) ---
module cpu_stack16_dmem(
    input  wire        clk,
    input  wire        we,
    input  wire [4:0]  addr,
    input  wire [15:0] wdata,
...
module cpu_stack16(
    ...
    wire [15:0] third;   // cell under NOS
    cpu_stack16_dstack u_dstack(
        .clk(clk),
