// =====================================================================
//  cpu_arm32.v
//  32-bit ARM-FLAVOURED CPU: full conditional execution (NZCV),
...
// --- cpu_arm32_cond : ARM condition check (gates EVERY instruction) ---
module cpu_arm32_cond(
    input  wire [3:0] cond,
    input  wire       fN,
    input  wire       fZ,
    input  wire       fC,
...
// --- cpu_arm32_decode : field extraction + condition-gated class decode ---
module cpu_arm32_decode(
    input  wire [31:0] ir,
...
// --- cpu_arm32_shifter : inline barrel shifter on operand 2 ---
// (operand-isolated: only live for register-form data processing)
module cpu_arm32_shifter(
    input  wire [31:0] rmv,
    ...
    always @(*) begin
        case (shtyp)
            ...
            default: begin                                  // ROR
                sh_out  = (sh_in >> sh_n) | (sh_in << (32 - sh_n));
                sh_cout = (sh_n == 5'd0) ? fC : sh_out[31];
            end
        endcase
    end
endmodule

// --- cpu_arm32_alu_arith : unified A+B+cin adder, ARM C/V convention ---
// (leaf of cpu_arm32_alu)
module cpu_arm32_alu_arith(
    input  wire [31:0] aa,
    input  wire [31:0] ab,
    input  wire [3:0]  op4,
    input  wire        fC,
    ...
    wire cin   = (op4 == 4'd2) | (op4 == 4'd3) | (op4 == 4'd12) ? 1'b1
               ...
               : 1'b0;
    wire [31:0] adA = inv_a ? ~aa : aa;
    wire [31:0] adB = inv_b ? ~ab : ab;
    wire [32:0] adS = {1'b0, adA} + {1'b0, adB} + {{32{1'b0}}, cin};
    assign ar_y = adS[31:0];
    assign ar_c = adS[32];                 // ARM: C = NOT borrow on subtract
    assign ar_v = (adA[31] == adB[31]) & (ar_y[31] != adA[31]);
endmodule

// --- cpu_arm32_alu_logic : AND/EOR/ORR/MOV/MVN/BIC + TST/TEQ (leaf of cpu_arm32_alu) ---
module cpu_arm32_alu_logic(
    input  wire [31:0] la,
    input  wire [31:0] lb,
    input  wire [3:0]  op4,
    output reg  [31:0] lg_y
);
...
// --- cpu_arm32_alu : data-processing ALU (operand-isolated arith + logic) ---
module cpu_arm32_alu(
    input  wire [31:0] rnv,
    input  wire [31:0] op2,
    input  wire [3:0]  op4,
    input  wire        fC,
    ...
    output wire [31:0] dp_y,
    output wire        ar_c,
    output wire        ar_v
);
    wire [31:0] aa = rnv & {32{sel_arith}};
    wire [31:0] ab = op2 & {32{sel_arith}};
    wire [31:0] ar_y;
    cpu_arm32_alu_arith u_arith(.aa(aa), .ab(ab), .op4(op4), .fC(fC),
                          .ar_y(ar_y), .ar_c(ar_c), .ar_v(ar_v));

    wire [31:0] la = rnv & {32{sel_logic}};
    wire [31:0] lb = op2 & {32{sel_logic}};
    wire [31:0] lg_y;
    cpu_arm32_alu_logic u_logic(.la(la), .lb(lb), .op4(op4), .lg_y(lg_y));

    assign dp_y = sel_arith ? ar_y : lg_y;
endmodule

// --- cpu_arm32_regfile : 16x32 register file, r14 = LR by convention ---
...
module cpu_arm32_regfile(
    ...
    input  wire [3:0]  rm,
    input  wire [3:0]  rd,
    output wire [31:0] rnv,
    output wire [31:0] rmv,
    output wire [31:0] rdv,
    input  wire [3:0]  dbg_sel,
    output wire [31:0] dbg_data
);
    reg [31:0] r [0:15];        // r14 = LR by convention
...
// --- cpu_arm32_dmem : 32-word data RAM (sync write, async read) ---
module cpu_arm32_dmem(
    input  wire        clk,
    input  wire        we,
    input  wire [4:0]  addr,
    input  wire [31:0] wdata,
...
// --- cpu_arm32_flags : NZCV status register (ARM S-bit semantics) ---
module cpu_arm32_flags(
    input  wire        clk,
    input  wire        rst,
    input  wire        we,
    input  wire [31:0] dp_y,
    input  wire        sel_arith,
    input  wire        ar_c,
    input  wire        ar_v,
    input  wire [1:0]  cls,
...
module cpu_arm32(
    ...
    cpu_arm32_cond u_cond(
        .cond(cond), .fN(fN), .fZ(fZ), .fC(fC), .fV(fV),
        .cond_pass(cond_pass)
    );
    cpu_arm32_decode u_decode(
    ...
    wire [31:0] rnv, rmv, rdv, dmem_rd, dp_y;
    wire [31:0] linkv = {{24{1'b0}}, pc1};   // BL: r14 <= pc+1
    wire rf_we = wr_gate & (dp_wr | is_ldr | is_bl);
    wire [3:0] rf_waddr = is_bl ? 4'd14 : rd;
    wire [31:0] rf_wdata = is_bl  ? linkv
                          : is_ldr ? dmem_rd
                          :          dp_y;
    cpu_arm32_regfile u_regfile(
        .clk(clk),
    ...
    wire [31:0] imm8w = {{24{1'b0}}, imm8};
    wire [31:0] op2 = (cls == 2'b00) ? sh_out : imm8w;
