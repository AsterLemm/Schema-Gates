// =====================================================================
//  cpu_riscv8.v
//  8-bit RV-LITE single-cycle CPU (RISC-V-flavoured load/store ISA).
...
// --- cpu_riscv8_decode : fields, instruction classes, ALU path selects ---
module cpu_riscv8_decode(
    input  wire [31:0] ir,
    output wire [3:0]  op,
    output wire [3:0]  rd,
    output wire [3:0]  rs1,
...
// --- cpu_riscv8_regfile : 16x8 register file, x0 reads as zero ---
module cpu_riscv8_regfile(
    input  wire        clk,
    input  wire        we,
    ...
    assign rdata1 = (raddr1 == 4'd0) ? {8{1'b0}} : x[raddr1];
    assign rdata2 = (raddr2 == 4'd0) ? {8{1'b0}} : x[raddr2];
...
// --- cpu_riscv8_alu_addsub : ADD / SUB path (leaf of cpu_riscv8_alu) ---
module cpu_riscv8_alu_addsub(
    input  wire [7:0] a,
    input  wire [7:0] b,
    input  wire        sub_en,
    output wire [7:0] y
...
// --- cpu_riscv8_alu_logic : AND / OR / XOR path (leaf of cpu_riscv8_alu) ---
module cpu_riscv8_alu_logic(
    input  wire [7:0] a,
    input  wire [7:0] b,
    input  wire [1:0]  lsel,
    output wire [7:0] y
...
// --- cpu_riscv8_alu_shift : SLL / SRL / SRA path (leaf of cpu_riscv8_alu) ---
module cpu_riscv8_alu_shift(
    input  wire [7:0] a,
    input  wire [2:0] n,
...
// --- cpu_riscv8_alu_cmp : SLT / SLTU path (leaf of cpu_riscv8_alu) ---
module cpu_riscv8_alu_cmp(
    input  wire [7:0] a,
    input  wire [7:0] b,
    input  wire [3:0]  funct,
    output wire [7:0] y
...
module cpu_riscv8_alu(
    ...
    wire [7:0] add_y;
    cpu_riscv8_alu_addsub u_path_add(.a(add_a), .b(add_b), .sub_en(add_sub_en), .y(add_y));

    wire [7:0] log_a = rs1v & {8{gate_logic}};
    wire [7:0] log_b = alu_b & {8{gate_logic}};
    wire [7:0] log_y;
    cpu_riscv8_alu_logic u_path_log(.a(log_a), .b(log_b), .lsel(lsel), .y(log_y));

    wire [7:0] shf_a = rs1v & {8{gate_shift}};
    wire [2:0] shf_n = rs2v[2:0] & {3{gate_shift}};
    wire [7:0] shf_y;
    cpu_riscv8_alu_shift u_path_shf(.a(shf_a), .n(shf_n), .funct(funct), .y(shf_y));

    wire [7:0] cmp_a = rs1v & {8{gate_cmp}};
    wire [7:0] cmp_b = alu_b & {8{gate_cmp}};
    wire [7:0] cmp_y;
    cpu_riscv8_alu_cmp u_path_cmp(.a(cmp_a), .b(cmp_b), .funct(funct), .y(cmp_y));
...
// --- cpu_riscv8_bru : branch resolve + next-PC select ---
module cpu_riscv8_bru(
    input  wire [7:0]  pc,
    ...
    wire [7:0] bcm_a = rs1v & {8{is_branch}};
    wire [7:0] bcm_b = rs2v & {8{is_branch}};
...
// --- cpu_riscv8_wbsel : write-back value select ---
module cpu_riscv8_wbsel(
    input  wire        is_lui,
    input  wire        is_load,
    input  wire        is_jal,
    input  wire        is_jalr,
    input  wire [15:0] imm16,
    input  wire [7:0] load_v,
    input  wire [7:0]  pc1,
    input  wire [7:0] alu_y,
    output wire [7:0] wb_val
);
    wire [7:0] pcret  = pc1[7:0];
    assign wb_val = is_lui          ? imm16[7:0]
                          : is_load          ? load_v
                          : (is_jal|is_jalr) ? pcret
                          :                    alu_y;
endmodule

// --- cpu_riscv8_dmem : 16-word data RAM (sync write, async read) ---
module cpu_riscv8_dmem(
    input  wire        clk,
    input  wire        we,
    input  wire [3:0]  addr,
    input  wire [7:0] wdata,
...
module cpu_riscv8(
    ...
    // ---- register file ----------------------------------------------
    wire [7:0] rs1v, rs2v, wb_val;
    cpu_riscv8_regfile u_regfile(
        .clk(clk),
    ...
    always @(posedge clk) begin
        out_valid <= 1'b0;
        if (rst) begin
            pc <= 8'd0; halted <= 1'b0; out_data <= {8{1'b0}};
