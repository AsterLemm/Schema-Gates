...
// --- cpu_vonneumann8_mem : the unified von Neumann memory (16x8) ---
...
// --- cpu_vonneumann8_alu_arith : ADD / SUB with carry view (leaf of cpu_vonneumann8_alu) ---
module cpu_vonneumann8_alu_arith(
    input  wire [7:0] ar_a,
    input  wire [7:0] ar_b,
    input  wire        is_sub,
    output wire [8:0]   ar_sum
);
    assign ar_sum = is_sub ? ({1'b0, ar_a} - {1'b0, ar_b})
                           : ({1'b0, ar_a} + {1'b0, ar_b});
endmodule

// --- cpu_vonneumann8_alu_logic : AND / OR / XOR (leaf of cpu_vonneumann8_alu) ---
module cpu_vonneumann8_alu_logic(
    input  wire [7:0] lg_a,
    input  wire [7:0] lg_b,
...
// --- cpu_vonneumann8_alu_shift : SHL / SHR with carry-out (leaf of cpu_vonneumann8_alu) ---
module cpu_vonneumann8_alu_shift(
    input  wire [7:0] sh_a,
    input  wire [3:0]  opcode,
    output wire [7:0] sh_y,
    output wire        sh_c
);
    assign sh_y = (opcode == 4'hC) ? {sh_a[6:0], 1'b0}
                :                    {1'b0, sh_a[7:1]};
    assign sh_c = (opcode == 4'hC) ? sh_a[7] : sh_a[0];
endmodule

// --- cpu_vonneumann8_alu : operand-isolated accumulator ALU (flagship technique) ---
// Each path's inputs are ANDed with its select so unused paths hold 0.
module cpu_vonneumann8_alu(
    input  wire [7:0] acc,
    ...
    cpu_vonneumann8_alu_arith u_arith(.ar_a(ar_a), .ar_b(ar_b), .is_sub(is_sub),
                          .ar_sum(ar_sum));

    wire [7:0] lg_a = acc       & {8{sel_logic}};
    wire [7:0] lg_b = mem_rdata & {8{sel_logic}};
    cpu_vonneumann8_alu_logic u_logic(.lg_a(lg_a), .lg_b(lg_b), .opcode(opcode),
                          .lg_y(lg_y));

    wire [7:0] sh_a = acc & {8{sel_shift}};
    cpu_vonneumann8_alu_shift u_shift(.sh_a(sh_a), .opcode(opcode),
                          .sh_y(sh_y), .sh_c(sh_c));
endmodule

module cpu_vonneumann8(
    input  wire        clk,
    input  wire        rst,
    input  wire        run,
    ...
    wire [7:0] mem_wdata = (~run) ? prog_data : acc;
    wire [7:0] mem_rdata;
    cpu_vonneumann8_mem u_mem(
        .clk(clk),
