...
// --- cpu_stack64_decode : fields + instruction class decode ---
module cpu_stack64_decode(
    input  wire [15:0] ir,
    output wire [3:0]  op,
    output wire [11:0] imm12,
    output wire [63:0] pushv,
    ...
    assign pushv = {{52{imm12[11]}}, imm12};   // PUSHI value
...
// --- cpu_stack64_alu_arith : ADD / SUB path (leaf of cpu_stack64_alu) ---
module cpu_stack64_alu_arith(
    input  wire [63:0] al_a,
    input  wire [63:0] al_b,
    input  wire [3:0]  op,
    output wire [63:0] y
);
...
// --- cpu_stack64_alu_logic : AND / OR / XOR path (leaf of cpu_stack64_alu) ---
module cpu_stack64_alu_logic(
    input  wire [63:0] al_a,
    input  wire [63:0] al_b,
    input  wire [3:0]  op,
    output wire [63:0] y
...
// --- cpu_stack64_alu : binop ALU on the two top cells (operand-isolated) ---
module cpu_stack64_alu(
    input  wire [63:0] nos,
    input  wire [63:0] tos,
    input  wire        is_binop,
    input  wire [3:0]  op,
    output wire [63:0] al_y
);
    wire [63:0] al_a = nos & {64{is_binop}};
    wire [63:0] al_b = tos & {64{is_binop}};
    wire [63:0] ar_y, lg_y;
    cpu_stack64_alu_arith u_arith(.al_a(al_a), .al_b(al_b), .op(op), .y(ar_y));
    cpu_stack64_alu_logic u_logic(.al_a(al_a), .al_b(al_b), .op(op), .y(lg_y));
...
// --- cpu_stack64_dstack : 16-deep data-stack spill RAM (cells BELOW nos) ---
module cpu_stack64_dstack(
    input  wire        clk,
    input  wire        we,
...
// --- cpu_stack64_rstack : 8-deep return stack (the dual-stack signature) ---
module cpu_stack64_rstack(
    input  wire        clk,
    input  wire        we,
...
// --- cpu_stack64_dmem : 32-word data RAM (sync write, async read) ---
module cpu_stack64_dmem(
    input  wire        clk,
    input  wire        we,
    input  wire [4:0]  addr,
    input  wire [63:0] wdata,
...
module cpu_stack64(
    ...
    wire [63:0] third;   // cell under NOS
    cpu_stack64_dstack u_dstack(
        .clk(clk),
