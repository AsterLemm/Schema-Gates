// =====================================================================
//  cpu_vonneumann16.v
//  16-bit VON NEUMANN accumulator CPU (multicycle, unified memory).
//  One 16x16 memory holds code AND data behind a single port,
...
// --- cpu_vonneumann16_mem : the unified von Neumann memory (16x16) ---
...
// --- cpu_vonneumann16_alu_arith : ADD / SUB with carry view (leaf of cpu_vonneumann16_alu) ---
module cpu_vonneumann16_alu_arith(
    input  wire [15:0] ar_a,
    input  wire [15:0] ar_b,
    input  wire        is_sub,
    output wire [16:0]   ar_sum
);
    assign ar_sum = is_sub ? ({1'b0, ar_a} - {1'b0, ar_b})
                           : ({1'b0, ar_a} + {1'b0, ar_b});
endmodule

// --- cpu_vonneumann16_alu_logic : AND / OR / XOR (leaf of cpu_vonneumann16_alu) ---
module cpu_vonneumann16_alu_logic(
...
// --- cpu_vonneumann16_alu_shift : SHL / SHR with carry-out (leaf of cpu_vonneumann16_alu) ---
module cpu_vonneumann16_alu_shift(
    input  wire [15:0] sh_a,
    input  wire [3:0]  opcode,
    output wire [15:0] sh_y,
    output wire        sh_c
);
    assign sh_y = (opcode == 4'hC) ? {sh_a[14:0], 1'b0}
                :                    {1'b0, sh_a[15:1]};
    assign sh_c = (opcode == 4'hC) ? sh_a[15] : sh_a[0];
endmodule

// --- cpu_vonneumann16_alu : operand-isolated accumulator ALU (flagship technique) ---
// Each path's inputs are ANDed with its select so unused paths hold 0.
module cpu_vonneumann16_alu(
    input  wire [15:0] acc,
    ...
    wire [15:0] ar_a = acc       & {16{sel_arith}};
    wire [15:0] ar_b = mem_rdata & {16{sel_arith}};
    cpu_vonneumann16_alu_arith u_arith(.ar_a(ar_a), .ar_b(ar_b), .is_sub(is_sub),
                          .ar_sum(ar_sum));

    wire [15:0] lg_a = acc       & {16{sel_logic}};
    wire [15:0] lg_b = mem_rdata & {16{sel_logic}};
    cpu_vonneumann16_alu_logic u_logic(.lg_a(lg_a), .lg_b(lg_b), .opcode(opcode),
                          .lg_y(lg_y));

    wire [15:0] sh_a = acc & {16{sel_shift}};
    cpu_vonneumann16_alu_shift u_shift(.sh_a(sh_a), .opcode(opcode),
                          .sh_y(sh_y), .sh_c(sh_c));
endmodule

module cpu_vonneumann16(
    input  wire        clk,
    input  wire        rst,
    input  wire        run,
    ...
    wire [15:0] mem_wdata = (~run) ? prog_data : acc;
    wire [15:0] mem_rdata;
    cpu_vonneumann16_mem u_mem(
        .clk(clk),
    ...
    always @(posedge clk) begin
        out_valid <= 1'b0;
        if (rst) begin
            state <= ST_FETCH; pc <= 4'd0; acc <= {16{1'b0}};
            ...
            case (state)
                ...
                ST_EXEC: begin
                    ...
                    case (cur_opcode)
                        ...
                        4'h3: begin acc <= ar_sum[15:0]; carry <= ar_sum[16]; end // ADD
                        4'h4: begin acc <= ar_sum[15:0]; carry <= ar_sum[16]; end // SUB (carry = borrow)
                        4'h5, 4'h6, 4'h7: acc <= lg_y;            // AND OR XOR
                        4'h8: acc <= {{12{1'b0}}, cur_operand}; // LDI
