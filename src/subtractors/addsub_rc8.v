// =====================================================================
//  addsub_rc8.v
//  8-bit add/sub on ripple adder (sub=1 -> a-b via b^1,cin=1).
//  Part of schema-gates by BITFries.
//  Self-contained: embeds every submodule it uses, down to leaf gates.
//  Target synthesizer: BITF-Synthesis Engine (Verilog -> SchemaGates).
// =====================================================================

module addsub_rc8(input [7:0] a, input [7:0] b, input sub, output [7:0] result, output cout, output ovf);
    ...
    wire [7:0] bx = b ^ {8{sub}};
    wire co;
    rcadd8 u(.a(a), .b(bx), .cin(sub), .sum(result), .cout(co));
    assign cout = co;
    assign ovf  = (a[7]==bx[7]) & (result[7]!=a[7]);
endmodule

module rcadd8(input [7:0] a, input [7:0] b, input cin, output [7:0] sum, output cout);
