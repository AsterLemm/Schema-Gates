// =====================================================================
//  sqrt_iter8.v
//  8-bit iterative integer square root (digit-by-digit).
...
module sqrt_iter8(input clk, input rst, input start, input [7:0] a, output reg [3:0] root, output reg done, output reg busy);
    ...
    reg [7:0] op; reg [7:0] res; reg [7:0] one;
    wire [7:0] ONE_INIT = 8'b1 << (8-2);
    wire [7:0] res_plus_one = res + one;   // add (structural-friendly)
    always @(posedge clk) begin
        if (rst) begin busy<=0; done<=0; root<=0; end
        else if (start) begin op<=a; res<=0; one<=ONE_INIT; busy<=1; done<=0; end
        else if (busy) begin
            if (one == 0) begin busy<=0; done<=1; root<=res[3:0]; end
            else begin
