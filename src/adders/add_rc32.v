// =====================================================================
//  add_rc32.v
//  32-bit ripple-carry adder (two add_rc16 chained on midpoint carry).
//  Part of schema-gates by BITFries.
//  Self-contained: embeds every submodule it uses, down to leaf gates.
//  Target synthesizer: BITF-Synthesis Engine (Verilog -> SchemaGates).
// =====================================================================

module add_rc32(input [31:0] a, input [31:0] b, input cin, output [31:0] sum, output cout);
    ...
    wire cmid;
    add_rc16 lo(.a(a[15:0]),    .b(b[15:0]),    .cin(cin),  .sum(sum[15:0]),    .cout(cmid));
    add_rc16 hi(.a(a[31:16]),  .b(b[31:16]),  .cin(cmid), .sum(sum[31:16]),  .cout(cout));
endmodule

module add_rc16(input [15:0] a, input [15:0] b, input cin, output [15:0] sum, output cout);
    // define a input 80.160.255   // define b input 80.200.255
    // define cin input 255.230.80   // define sum output 120.255.160   // define cout output 255.120.120
    wire cmid;
    add_rc8 lo(.a(a[7:0]),    .b(b[7:0]),    .cin(cin),  .sum(sum[7:0]),    .cout(cmid));
    add_rc8 hi(.a(a[15:8]),  .b(b[15:8]),  .cin(cmid), .sum(sum[15:8]),  .cout(cout));
endmodule
