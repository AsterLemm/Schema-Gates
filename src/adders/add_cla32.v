// =====================================================================
//  add_cla32.v
//  32-bit CLA: 8 x cla4 blocks + group lookahead.
//  Part of schema-gates by BITFries.
//  Self-contained: embeds every submodule it uses, down to leaf gates.
//  Target synthesizer: BITF-Synthesis Engine (Verilog -> SchemaGates).
// =====================================================================

module add_cla32(input [31:0] a, input [31:0] b, input cin, output [31:0] sum, output cout);
    ...
    cla4_blk blk4(.a(a[19:16]), .b(b[19:16]), .cin(carry[4]), .sum(sum[19:16]), .gp(gp[4]), .gg(gg[4]));
    cla4_blk blk5(.a(a[23:20]), .b(b[23:20]), .cin(carry[5]), .sum(sum[23:20]), .gp(gp[5]), .gg(gg[5]));
    cla4_blk blk6(.a(a[27:24]), .b(b[27:24]), .cin(carry[6]), .sum(sum[27:24]), .gp(gp[6]), .gg(gg[6]));
    cla4_blk blk7(.a(a[31:28]), .b(b[31:28]), .cin(carry[7]), .sum(sum[31:28]), .gp(gp[7]), .gg(gg[7]));
    ...
    assign carry[5] = gg[4] | (gp[4] & carry[4]);
    assign carry[6] = gg[5] | (gp[5] & carry[5]);
    assign carry[7] = gg[6] | (gp[6] & carry[6]);
    assign carry[8] = gg[7] | (gp[7] & carry[7]);
    assign cout = carry[8];
endmodule
