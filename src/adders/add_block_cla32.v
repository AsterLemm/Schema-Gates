// =====================================================================
//  add_block_cla32.v
//  32-bit block CLA: 8 CLA-4 blocks, carries rippled between blocks.
//  Part of schema-gates by BITFries.
//  Self-contained: embeds every submodule it uses, down to leaf gates.
//  Target synthesizer: BITF-Synthesis Engine (Verilog -> SchemaGates).
// =====================================================================

module add_block_cla32(input [31:0] a, input [31:0] b, input cin, output [31:0] sum, output cout);
    ...
    add_cla4_unit blk4(.a(a[19:16]),.b(b[19:16]),.cin(c[4]),.sum(sum[19:16]),.cout(c[5]));
    add_cla4_unit blk5(.a(a[23:20]),.b(b[23:20]),.cin(c[5]),.sum(sum[23:20]),.cout(c[6]));
    add_cla4_unit blk6(.a(a[27:24]),.b(b[27:24]),.cin(c[6]),.sum(sum[27:24]),.cout(c[7]));
    add_cla4_unit blk7(.a(a[31:28]),.b(b[31:28]),.cin(c[7]),.sum(sum[31:28]),.cout(c[8]));
    assign cout=c[8];
endmodule
