...
module gold_cpu_x86_32(
    ...
    wire [31:0] rv = regs[rsel];   // destination operand (read)
    wire [31:0] mv = regs[msel];   // source operand
    ...
    wire [32:0] ar_full = ar_subop ? ({1'b0, ar_a} - {1'b0, ar_b})
                                    : ({1'b0, ar_a} + {1'b0, ar_b});
    wire [31:0] ar_y = ar_full[31:0];
    wire ar_c = ar_full[32];                       // carry / borrow
    wire ar_v = ar_subop ? ((ar_a[31] != ar_b[31]) & (ar_y[31] != ar_a[31]))
                         : ((ar_a[31] == ar_b[31]) & (ar_y[31] != ar_a[31]));

    // logic path
    wire [31:0] lg_a = rv & {32{sel_logic}};
    wire [31:0] lg_b = mv & {32{sel_logic}};
    wire [31:0] lg_y = is_and ? (lg_a & lg_b)
                        : is_or  ? (lg_a | lg_b)
                        : is_xor ? (lg_a ^ lg_b)
                        :          (~lg_a);          // NOT
    ...
    wire [31:0] sh_y = (msel == 2'd0) ? {sh_a[30:0], 1'b0}        // SHL
                        : (msel == 2'd1) ? {1'b0, sh_a[31:1]}        // SHR
                        : (msel == 2'd2) ? {sh_a[31], sh_a[31:1]}    // SAR
                        :                  {sh_a[30:0], sh_a[31]};   // ROL
    wire sh_c = (msel == 2'd0) ? sh_a[31]
              : (msel == 2'd3) ? sh_a[31]
              :                  sh_a[0];
    wire sh_v = (msel == 2'd0) ? (sh_y[31] ^ sh_c)    // SHL: OF = CF^MSB(result)
    ...
    always @(*) begin case (cond)
        ...
        4'd13: ctaken = (regs[2] != {32{1'b0}});  // LOOP looks at CX-1... see EXEC
        default: ctaken = 1'b0;          // 14/15 never
    endcase end

    wire [31:0] cx_dec = regs[2] - {{31{1'b0}}, 1'b1};
    wire loop_take = (cx_dec != {32{1'b0}});

    wire [31:0] stk_top = ram[sp[4:0]];
    ...
    always @(posedge clk) begin
        ...
        if (rst) begin
            ...
            case (op)
                ...
                4'h1: regs[rsel] <= {{24{1'b0}}, imm8};         // MOV r,imm8
                ...
                4'hE: regs[rsel] <= {rv[23:0], imm8};     // MOVH (shift-in byte)
                4'hF: case (msel)
