...
module gold_cpu_x86_4(
    ...
    wire [3:0] rv = regs[rsel];   // destination operand (read)
    wire [3:0] mv = regs[msel];   // source operand
    ...
    wire [4:0] ar_full = ar_subop ? ({1'b0, ar_a} - {1'b0, ar_b})
                                    : ({1'b0, ar_a} + {1'b0, ar_b});
    wire [3:0] ar_y = ar_full[3:0];
    wire ar_c = ar_full[4];                       // carry / borrow
    wire ar_v = ar_subop ? ((ar_a[3] != ar_b[3]) & (ar_y[3] != ar_a[3]))
                         : ((ar_a[3] == ar_b[3]) & (ar_y[3] != ar_a[3]));

    // logic path
    wire [3:0] lg_a = rv & {4{sel_logic}};
    wire [3:0] lg_b = mv & {4{sel_logic}};
    wire [3:0] lg_y = is_and ? (lg_a & lg_b)
                        : is_or  ? (lg_a | lg_b)
                        : is_xor ? (lg_a ^ lg_b)
                        :          (~lg_a);          // NOT
    ...
    wire [3:0] sh_y = (msel == 2'd0) ? {sh_a[2:0], 1'b0}        // SHL
                        : (msel == 2'd1) ? {1'b0, sh_a[3:1]}        // SHR
                        : (msel == 2'd2) ? {sh_a[3], sh_a[3:1]}    // SAR
                        :                  {sh_a[2:0], sh_a[3]};   // ROL
    wire sh_c = (msel == 2'd0) ? sh_a[3]
              : (msel == 2'd3) ? sh_a[3]
              :                  sh_a[0];
    wire sh_v = (msel == 2'd0) ? (sh_y[3] ^ sh_c)    // SHL: OF = CF^MSB(result)
    ...
    always @(*) begin case (cond)
        ...
        4'd13: ctaken = (regs[2] != {4{1'b0}});  // LOOP looks at CX-1... see EXEC
        default: ctaken = 1'b0;          // 14/15 never
    endcase end

    wire [3:0] cx_dec = regs[2] - {{3{1'b0}}, 1'b1};
    wire loop_take = (cx_dec != {4{1'b0}});

    wire [3:0] stk_top = ram[sp[4:0]];
    ...
    always @(posedge clk) begin
        ...
        if (rst) begin
            ...
            case (op)
                ...
                4'h1: regs[rsel] <= imm8[3:0];         // MOV r,imm8
                ...
                4'hC: case (msel)
                    ...
                    2'd2: begin                                                 // CALL
                        ram[sp[4:0] - 5'd1] <= pc1w[3:0];
                        sp <= sp - 6'd1; pc <= imm8;
                    end
                    2'd3: begin pc <= {4'd0, stk_top}; sp <= sp + 6'd1; end // RET
                endcase
                4'hD: begin                                    // Jcc / LOOP
                    if (is_loop) begin
                ...
                4'hE: regs[rsel] <= imm8[3:0];                 // MOVH == MOV on W=4
                4'hF: case (msel)
