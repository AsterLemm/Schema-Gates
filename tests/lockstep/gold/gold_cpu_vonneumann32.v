...
module gold_cpu_vonneumann32(
    ...
    wire [31:0] mem_rdata = mem[mem_raddr];
    ...
    wire [32:0]   ar_sum = is_sub ? ({1'b0, ar_a} - {1'b0, ar_b})
                                   : ({1'b0, ar_a} + {1'b0, ar_b});

    wire [31:0] lg_a = acc       & {32{sel_logic}};
    wire [31:0] lg_b = mem_rdata & {32{sel_logic}};
    wire [31:0] lg_y = (opcode == 4'h5) ? (lg_a & lg_b)
                        : (opcode == 4'h6) ? (lg_a | lg_b)
                        :                    (lg_a ^ lg_b);

    wire [31:0] sh_a = acc & {32{sel_shift}};
    wire [31:0] sh_y = (opcode == 4'hC) ? {sh_a[30:0], 1'b0}
                        :                    {1'b0, sh_a[31:1]};
    wire sh_c = (opcode == 4'hC) ? sh_a[31] : sh_a[0];
