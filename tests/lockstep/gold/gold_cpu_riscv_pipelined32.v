...
module gold_cpu_riscv_pipelined32(
    input  wire        clk,
    input  wire        rst,
    ...
    wire [31:0] immv = {{16{imm16[15]}}, imm16};   // sign-extended
    ...
    wire [31:0] rf1 = (rs1 == 4'd0) ? {32{1'b0}}
                      : (memwb_v && memwb_rw && memwb_rd == rs1) ? memwb_val
                      : x[rs1];
    wire [31:0] rf2 = (rs2 == 4'd0) ? {32{1'b0}}
    ...
    wire [31:0] fwd1 = (exmem_v && exmem_rw && exmem_rd != 4'd0
                        && exmem_rd == idex_rs1) ? exmem_val
                      : (memwb_v && memwb_rw && memwb_rd != 4'd0
                        && memwb_rd == idex_rs1) ? memwb_val
                      : idex_rs1v;
    wire [31:0] fwd2 = (exmem_v && exmem_rw && exmem_rd != 4'd0
    ...
    wire [31:0] ex_b = idex_alur ? fwd2 : idex_imm;
    ...
    wire [31:0] add_a = fwd1 & {32{gate_add}};
    wire [31:0] add_b = ex_b & {32{gate_add}};
    wire           add_sub_en = (idex_funct == 4'd1);   // R-type SUB
    wire [31:0] add_y = add_sub_en ? (add_a - add_b) : (add_a + add_b);
    
    wire [31:0] log_a = fwd1 & {32{gate_logic}};
    wire [31:0] log_b = ex_b & {32{gate_logic}};
    wire [31:0] log_y = (idex_lsel == 2'd0) ? (log_a & log_b)
                         : (idex_lsel == 2'd1) ? (log_a | log_b)
                         :                  (log_a ^ log_b);
    
    wire [31:0] shf_a = fwd1 & {32{gate_shift}};
    wire [4:0] shf_n = fwd2[4:0] & {5{gate_shift}};
    // SRA on its own signed wire: inside a ?: chain with unsigned
    // branches, >>> would silently degrade to a logical shift
    wire signed [31:0] shf_as  = $signed(shf_a);
    wire        [31:0] shf_sra = shf_as >>> shf_n;
    wire [31:0] shf_y = (idex_funct == 4'd5) ? (shf_a <<  shf_n)
                         : (idex_funct == 4'd6) ? (shf_a >>  shf_n)
                         :                  shf_sra;
    
    wire [31:0] cmp_a = fwd1 & {32{gate_cmp}};
    wire [31:0] cmp_b = ex_b & {32{gate_cmp}};
    wire           cmp_lt = (idex_funct == 4'd8)
                          ? ($signed(cmp_a) < $signed(cmp_b))
                          : (cmp_a < cmp_b);
    wire [31:0] cmp_y = {{31{1'b0}}, cmp_lt};
    
    // path merge: one-hot by construction (decoder-style front mux)
    wire [31:0] alu_y = (add_y & {32{gate_add}})
                         | (log_y & {32{gate_logic}})
                         | (shf_y & {32{gate_shift}})
                         | (cmp_y & {32{gate_cmp}});
    ...
    wire [31:0] ex_wbv = idex_lui              ? {idex_imm16, {16{1'b0}}}
    ...
    // =================================================================
    reg [31:0] dmem [0:15];
    wire [31:0] mem_rd = dmem[exmem_val[3:0]];
