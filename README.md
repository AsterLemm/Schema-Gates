...
An educational Verilog logic-gate layout library by **BITFries**, designed to be
fed into the **BITF-Synth Engine** synthesizer (Verilog → HTML schematic
viewer). Every module is written to be readable, hierarchical, and traceable.

It has 1140 modules across 29 families, every file elaborates under Icarus Verilog.
...
Each structural core is verified against the corresponding `*`/`/` golden, a
meaningful test precisely because the device under test contains no such operator
(exhaustive at 4/8-bit where simulation-feasible, bounded random for 16/32-bit).
